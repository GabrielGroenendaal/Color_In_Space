...
export class Planet extends SolarSystemBody {
      ...
      constructor(options) {
            options.radius = options.radius || Util.randomSize(10, 20);
      ...
      explode() {
            ...
            while (count < this.num_of_giblets) {
                  ...
                  new_pos[0] = Math.cos(new_angle) * this.radius / 4;
                  new_pos[1] = Math.sin(new_angle) * this.radius / 4;
                  this.solar_system.add(new ExplosionTrail({
                        pos: this.pos,
                        solar_system: this.solar_system,
                        size: this.radius * .75,
                        color: JSON.parse(JSON.stringify(this.explode_trail_color)),
